...
function count_ssh_login_attempts_by_user_by_criteria() {
...
function count_ssh_failed_passwords_by_user() {
  printf "Number of invalid password attempts per user:\n"
  count_ssh_login_attempts_by_user_by_criteria "${FAILED_PASSWORD_MARKER}"
...
function main() {
  {
    ...
    count_ssh_login_attempts_by_user_by_criteria "${SUCCESSFUL_LOGIN_MARKER}"
  } >>"${OUTPUT_FILE}"

  {
    printf "Number of failed SSH logins:\n"
    count_ssh_login_attempts_by_criteria "${FAILED_LOGIN_MARKER}"
    count_ssh_login_attempts_by_user_by_criteria "${FAILED_LOGIN_MARKER}"
  } >>"${OUTPUT_FILE}"

  {
    count_ssh_failed_passwords_by_user
